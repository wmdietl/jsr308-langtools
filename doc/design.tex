...
expression.  For annotations within a field initializer or a
static initializer block, the compiled representation might be in an
\code{<init>} or \code{<clinit>} method, or in a helper method called by them.
...
An \RuntimeInOrVisibleTypeAnnotations attribute containing a
\code{offset\_target} lives in the attributes table of a
...
An \RuntimeInOrVisibleTypeAnnotations attribute containing a
\code{localvar\_target} lives in the attributes table of a
\code{method\_info} structure.
...
An \RuntimeInOrVisibleTypeAnnotations attribute containing a
\code{methodparam\_target} lives in the attributes table of a
\code{method\_info} structure.

\subsubsection{Method receivers\label{class-file:ext:ri:receiver}}

% TODO: Specify an empty_target here!
When the annotation's target is a method receiver (or an inner class
constructor receiver), \code{target\_info} is empty.

% TODO: Make the following paragraph consistent with the rest
An \RuntimeInOrVisibleTypeAnnotations attribute targeting
a method receiver lives in the attributes table of a
\code{method\_info} structure.
...
result, or a field, \code{target\_info} is empty.

Section~\ref{class-file:ext:ri:typearg} describes the \code{target\_info}
if the target is a type argument.

% TODO: Make the following paragraph consistent with the rest
An \RuntimeInOrVisibleTypeAnnotations attribute targeting a field
type lives in the attributes table of a \code{field\_info} structure.
An \RuntimeInOrVisibleTypeAnnotations attribute targeting a method return
type lives in the attributes table of a \code{field\_info} structure.
...
An \RuntimeInOrVisibleTypeAnnotations attribute containing a
\code{typeparam\_bound\_target} lives in the attributes table of a
...
An \RuntimeInOrVisibleTypeAnnotations attribute containing a
\code{supertype\_target} lives in the attributes table of a
...
An \RuntimeInOrVisibleTypeAnnotations attribute containing a
\code{wildcard\_bound\_target} lives in the attributes table of
...
\begin{Verbatim}
  ...
  class MyMap<T,U> {
    // The method requires that the map's keys be non-null, but the values may be arbitrary.
    public void requiresNonNullKeys() MyMap<@NonNull T,U> { ... }
  }
\end{Verbatim}
...
% LocalWords:  Nonnegative typedefs unabbrev reusability typedef getstatic
% LocalWords:  QualifiedIdentifier toArray localvar methodparam typeparam
% LocalWords:  typearg ClassFile
