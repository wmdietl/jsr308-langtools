...
cast & \code{CAST}
& \code{0x43} & \S\ref{class-file:ext:ri:cast} \\
type test (\code{instanceof}) & \code{INSTANCEOF}
& \code{0x44} & \S\ref{class-file:ext:ri:cast} \\
object creation (\code{new}) & \code{NEW}
& \code{0x45} & \S\ref{class-file:ext:ri:cast} \\
type argument in constructor call & \code{CONSTRUCTOR\_INVOCATION\_TYPE\_ARGUMENT}
& \code{0x46} & \S\ref{class-file:ext:ri:con-typearg} \\
type argument in method call & \code{METHOD\_INVOCATION\_TYPE\_ARGUMENT}
& \code{0x47} & \S\ref{class-file:ext:ri:con-typearg} \\
lambda formal parameter type & \code{LAMBDA\_FORMAL\_PARAMETER}
& \code{0x48} & \S\ref{class-file:ext:ri:lambda-formal-parameter} \\
method reference & \code{METHOD\_REFERENCE}
& \code{0x49} & \S\ref{class-file:ext:ri:mref} \\
type argument in method reference & \code{METHOD\_REFERENCE\_TYPE\_ARGUMENT}
& \code{0x50} & \S\ref{class-file:ext:ri:mref-typearg} \\
\hline
\end{tabular}
\end{center}
%BEGIN LATEX
...
TODO: Constrain \code{method\_formal\_parameter\_index} w.r.t.\ the number of
      formal parameter descriptors in the method descriptor (JVMS 4.3.3) of the
...
%      inferred formal parameter types for lambda -- like diamond (same issues), so
...
% formal parameter or return value) and to query it directly.
...
the formal parameter \emph{declarations}, just as in Java SE 7.  It does not return
...
whose formal parameter type is annotated as \code{@Readonly}, but the
