...
array type,
\code{reference\_info} contains what it normally would for the a raw,
non-array type
(e.g., \code{offset} for an annotation on a type argument in a
typecast, see Section~\ref{class-file:ext:ri:typecast}), {\em plus} the following fields at the end:
...
The \code{location} field is a list of numbers that encodes a path, in the
type, to the annotation.

A parameterized or array type can be viewed as a tree.  For a parameterized
...
, where each node is a type and its children
are the type arguments in case of parameterized.    Specifically,
