...
the Java language syntax (Section~\ref{syntax}),
...
\item
  An annotation on a type parameter wildcard appears before the wildcard,
...
declaration.  This appears before the declared name, as in \code{class
  MyClass<@Immutable T> \ttlcb\ ... \ttrcb}.  This is a declaration
annotation, not a type annotation, but it was an oversight in JSR
175~\cite{JSR175} and JSR 308 is taking the opportunity to correct it.
...
exception type in \code{throws}                 & \code{0x12}, \code{0x13}\rlunused & \S\ref{class-file:ext:ri:throws} \\
...
exception parameter                             & \code{0x1A}, \code{0x1B} & \S\ref{class-file:ext:ri:localvar} \\
...
Sections~\ref{type-annotation-locations} and~\ref{grammar}.
...
% LocalWords:  QualifiedTypeList MethodDeclaratorRest IdentifierSuffix
...
% LocalWords:  QualifiedType toArray localvar methodparam typeparam
