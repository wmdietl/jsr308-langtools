...
that are defined in Java SE 6:  
\code{ANNOTATION\_TYPE},
\code{CONSTRUCTOR},
\code{FIELD},
\code{LOCAL\_VARIABLE},
\code{METHOD},
\code{PACKAGE},
\code{PARAMETER}, and
\code{TYPE}.
...
method return type generic/array                & \code{0x0B} & +\S\ref{class-file:ext:ri:typearg} \\
method parameter                                & \code{0x0C} & \S\ref{class-file:ext:ri:parameters} \\
method parameter generic/array                  & \code{0x0D} & +\S\ref{class-file:ext:ri:typearg} \\
field                                           & \code{0x0E} & \S\ref{class-file:ext:ri:return} \\
field generic/array                             & \code{0x0F} & +\S\ref{class-file:ext:ri:typearg} \\
...
class literal generic/array                     & \code{0x1F} & +\S\ref{class-file:ext:ri:typearg} \\
...
the \code{new} bytecode emitted for the object creation expression, or
...
\code{reference\_info} indicates which of the method's formal
parameters is being annotated:
...
% A motivation for generic annotations on the receiver is the annotation on
% toArray, which (in the nullness case) should be:
%   public @Polynull java.lang.Object [] toArray() ArrayList<@PolyNull E> { ... }
...
result, or a field, \code{reference\_info} is empty.

Section~\ref{class-file:ext:ri:typearg} describes the \code{reference\_info}
if the target is a type argument.
...
A type can be viewed as a tree, where each node is a type and its children
are the type arguments in case of parameterized.  The \code{location} field
encodes the path to the annotation from the raw type.  Specifically,
the $i$th item in \code{location} denotes the child index at the $i$th level.
The \code{location\_length} field is the length of the \code{location} field,
which is the depth of the tree.

For example, the type \code{Map<@B String, @C List<@D String>>} can be
viewed as the following tree:
...
Thus the annotations will have the following values:
...
in another sample field declaration.
...
in which these plug-ins exposed bugs in real programmers.
...
style!), it is treated as if it applies to all locations that were possible
in Java SE 6.  
If there is no \code{@Target} meta-annotation, a Java SE 6
and a Java SE 7 put the same 
annotations in the \code{.class} file, and no annotation is placed
on multiple locations nor appears more than once in the \code{.class} file.
If you wish to write a type annotation, you must meta-annotate its
definition with \code{@Target(ElementType.TYPE\_USE)}.
...
\noindent
the \code{@Deprecated} annotation applies to only the \code{foo} method,
not the \code{int} data type.
