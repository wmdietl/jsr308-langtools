% If you edit this file, also edit file jsr308-changes.html
...
\urldef{\jlsEightEightFour}\url{https://docs.oracle.com/javase/specs/jls/se7/html/jls-8.html#jls-8.8.4}
\urldef{\jlsFourteenTwenty}\url{https://docs.oracle.com/javase/specs/jls/se7/html/jls-14.html#jls-14.20}
\urldef{\jlsFifteenEightThree}\url{https://docs.oracle.com/javase/specs/jls/se7/html/jls-15.html#jls-15.8.3}
\urldef{\jlsFifteenEightFour}\url{https://docs.oracle.com/javase/specs/jls/se7/html/jls-15.html#jls-15.8.4}
\urldef{\jlsFifteenNine}\url{https://docs.oracle.com/javase/specs/jls/se7/html/jls-15.html#jls-15.9}
...
It contains the
\ahref{https://checkerframework.org/jsr308/specification/java-annotation-design.html}{latest version}
% latest version
...
% TODO: add a link to the above.

%HEVEA This document is available in PDF format at \myurl{https://checkerframework.org/jsr308/specification/java-annotation-design.pdf}.
...
the Public Review specification of JSR 308 at jcp.org.
% TODO: reinstate this link
% \ahref{https://checkerframework.org/jsr308/specification/jsr308.pdf}{JLS and JVMS changes}.
%
...
see \myurl{https://checkerframework.org/jsr308/jsr308-faq.html}.
...
\code{\emph{Supertype}.this}, and there can be multiple receivers for
...
% This section
% gives changes to the \ahref{https://docs.oracle.com/javase/specs/jls/se7/html/jls-18.html}{grammar} of the Java
...
% \\
% \nt{LocalVariableDeclarationStatement}: \\
% \qquad        \multi{\nt{VariableModifier}} \nt{\new{Unann}Type} \nt{VariableDeclarators} \term{;} \\
% \\
% \nt{Resource}: \\
...
% \\
% \nt{AnnotationTypeElementRest}: \\
% \qquad       \nt{\new{Unann}Type} \nt{Identifier} \nt{AnnotationMethodOrConstantRest} \term{;} \\
% \qquad       \ldots \\
% \end{tabbing}
% 
...
% % but NOT in annotation (\<@\emph{Foo}>)!
% The JLS grammar uses \nt{QualifiedIdentifer} in such places, but the
% grammar also uses \nt{QualifiedIdentifer} in \<package> statements, where
...
% \qquad            \nt{FormalParameters} \opt{\term{throws} \nt{Qualified\new{Type}List}}   \term{;}
% \end{tabbing}
% 
% The CatchType rule allows each exception type in a union of types (JLS
% \ahref{\jlsFourteenTwenty}{\S14.20}) to be annotated independently.  There
% is no syntax to indicate that an annotation applies to every exception type
% in a union of types.
% % the programmer would have to duplicate the annotation for each disjunct.
% % , but a higher level tool might choose to act as if the
% % first annotation in a union of types applies to every type in the union.
% 
% 
% 
% % \item
...
% Two such changes (in \nt{MethodDeclaratorRest} and
% \nt{InterfaceMethodDeclaratorRest}) already appeared above in
...
type.  For example, \code{var1} has
...
% The alternative has a much uglier and more complex implementation.
...
% Finally, use of fully-qualified names in Java source code is uncommon.
...
write a \code{@Target} meta-annotation indicating that an annotation, such
...
% Are Colin's examples about effect systems a counterexample?
...
decompiled code, as in ``\code{@FTAnno @FTAnno Object field6;}''.
...
(\myurl{https://checkerframework.org/}) is
...
% , which are described in Section~\ref{class-file:ext}.
...
% are those in Figure~\ref{tbl:target_types} with
have
\<target\_type> $\ge$ 0x40.
% ; see below for details.

%% Avoid saying everywhere that something *doesn't* change.
% No changes are made to the StackMapTable that stores the types of elements
% on the stack.
% % Doing so would eliminate the need for pluggable type-checking on byte code
...
%   % 
%   %% This implementation detail is out of place for this document.
...
% expression.  
% For a constructor/method invocation, \code{type\_argument\_index} specifies
% the index of a type argument in the list of explicit non-wildcard type
% arguments, such as
% \code{x.<@Foo String>m()} or \code{new <@Foo String> @Bar A()}.
% 
% It might seem odd that instanceof uses \code{target\_info} of
% \code{offset\_target}, whereas cast uses \code{type\_argument\_index},
% which has an additional \code{type\_argument\_index} field.  The reason is
...
% 
% 
% \subsection{Examples of the \code{type\_path} structure\label{classfile-examples-type_path}}
% 
% Figure~\ref{tbl:locations} shows some examples of the \code{type\_path}
...
\myurl{https://checkerframework.org/jsr308/}.  These checkers
...
(\myurl{https://checkerframework.org/})
...
(\myurl{http://ricken.us/research/laptjavac/}) is a version
...
javac) permits subtyping of annotations (\myurl{http://ricken.us/research/xajavac/}).
...
% types'' (\myurl{https://checkerframework.org/jsr308/java-type-qualifiers.pdf}),
...
% (\myurl{https://checkerframework.org/annotation-file-utilities/annotation-file-format.pdf}), describes a
...
(\myurl{https://groups.google.com/forum/#!forum/jsr308-discuss}) provided
...
% LocalWords:  QualifiedType toArray localvar ReadWrite
% LocalWords:  ClassFile clinit StackMapTable StaticNested boolean var1 
...
% LocalWords:  SillyAnnotation 
% LocalWords:  casted useE1 useE2 pack200 unpack200 
% LocalWords:  Mahmood theReceiver foo 
% LocalWords:  innerMethod 
% LocalWords:  multi classfiles MyType
% LocalWords:  TypeAnnotations LocalVariableDeclarationStatement
% LocalWords:  VariableDeclarators AnnotationTypeElementRest CatchType
% LocalWords:  AnnotationMethodOrConstantRest QualifiedIdentifer 
% LocalWords:  QualifiedIdentiferList UnannQualifiedType 
% LocalWords:  StaticNestedClass 
% LocalWords:  RetentionPolicy FTAnno MTAnno field6 
