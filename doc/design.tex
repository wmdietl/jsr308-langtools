...
\item
The receiver may be expressed explicitly, by changing the definition of 
``\nt{FormalParameters}''.
...
\code{@Target(ElementType.TYPE\_USE)}) is permitted to be written on a 
a class declaration or a type parameter declaration.  Strictly speaking,
...
constructor or static method), or a field, \code{target\_info} is empty.
...
\targetinfocontents{the bound of a wildcard}{wildcard\_bound\_target}

\preverbnegspace
\begin{Verbatim}
    wildcard_bound_target {
        u2 wildcard_location_type;   // The location of the wildcard, values as for target_type (Section 3.2)
        {
            ...
        } target_info;  // uniquely identifies the targeted program element, see Section 3.3
    }
...
Here are examples (see Section~\ref{class-file:ext:ri:typearg} for
an explanation of \<location\_length> and \<location>):

\noindent
\code{@A} in \code{void m(List<?\ extends @A String> lst) \ttlcb...\ttrcb} is described as:
...
\begin{Verbatim}
   ...
   target_info =
   {
     wildcard_location_type = METHOD_PARAMETER_COMPONENT (0x0D)
     wildcard_location =
     {
       u1 parameter_index = 0;
       // the first type argument of List
       location_length = 1;
       location = { 0 };
     }
   }
...
\noindent
\code{@B} in \code{Map<Object, ?\ extends List<@B String>> newMap() \ttlcb...\ttrcb} is described as:

\preverbnegspace
\begin{Verbatim}
   target_type = WILDCARD_BOUND_COMPONENT
   target_info =
   {
     // location of wildcard: type argument of the return type
     wildcard_location_type = RETURN_TYPE_COMPONENT;
     wildcard_location =
     {
       // the second type argument of Map
       location_length = 1;
       location = { 1 };
     }
     // the first type argument of List (the generic information of the annotation itself)
     location_length = 1;
     location = { 0 };
   }
\end{Verbatim}
...
\targetinfocontentspartial{within a parameterized type,
...
It may be desirable to write an annotation that should should apply to all
...
% LocalWords:  0x06 0x07 0x08 0x09 0x0A 0x0B 0x0C 0x0D 0x0E 0x0F 0x10
% LocalWords:  0x11 0x12 0x13 0x14 0x15 0x16 0x17 0x18 0x19 0x1A 0x1B
% LocalWords:  0x1C 0x1D 0x1E 0x1F 0x20 0x21 0x22 0x23 0x24 0x25 308's
