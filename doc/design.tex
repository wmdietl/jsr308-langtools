...
\item
  ...
  Then, no special
  syntax is needed for expressing the receiver type annotation:  it is simply an
  annotation on the type of the \code{this} parameter.
  It is not permitted to write a declaration annotation on the
  receiver (<tt>this</tt>) formal parameter declaration.
...
\end{Verbatim}
  ...
  possible to annotate its receiver.  If you need to do so, re-write the
...
Annotations that target instructions are are those in
...
See Section~\ref{class-file:ext:type_path} for handling of generic
...
superclass type.  Otherwise, \code{supertype\_index} specifies the index of the
...
\targetinfocontentspartial{a method parameter type} contains one \code{method\_parameter\_target}, which
...
\attributelocation{containing a \code{method\_parameter\_target}}{\code{method\_info} structure}

TODO: Constrain \code{method\_parameter\_index} w.r.t.\ the number of
      parameter descriptors in the method descriptor (JVMS 4.3.3) of the
      enclosing \code{method\_info} structure.
...
\targetinfocontents{a cast, an \code{instanceof}
...
the cast, the \code{instanceof} bytecode emitted for the type tests,
...
\attributelocation{containing a \code{offset\_target}}{\code{Code} attribute}
...
a method call}{invocation\_type\_argument\_target}
...
\attributelocation{containing a \code{invocation\_type\_argument\_target}}{\code{Code} attribute}
...
\code{@C}       & \typeargpath{0}{\wildcardpath{\emptypath}}  \\
\code{@D}       & \typeargpath{1}{\emptypath}  \\
\code{@E}       & \typeargpath{1}{\typeargpath{0}{\emptypath}}  \\
\code{@F}       & \emptypath \\   
\code{@G}       & \arraypath{\emptypath} \\                  
\code{@H}       & \arraypath{\arraypath{\emptypath}} \\                  
\code{@I}       & \arraypath{\arraypath{\arraypath{\emptypath}}} \\                  
\code{@J}       & \emptypath \\   
\code{@K}       & \innerpath{\emptypath} \\                  
\code{@L}       & \innerpath{\innerpath{\emptypath}} \\                  
\code{@M}       & \innerpath{\innerpath{\innerpath{\emptypath}}} \\                  
\hline
\end{tabular}
...
struct type_path {
    ...
    union {
        struct empty_path {};
        struct array_type_path { type_path element_path; };
        struct inner_type_path { type_path outer_path; };
        struct wildcard_path { type_path bound_path; };
        struct type_argument_path { u1 parameter_index; type_path parameter_path; };
    }
}
\end{Verbatim}
...
the \code{type\_path} structure represents a path in that tree, from the
top down or the ``outside in''.

The \code{type\_path} structure is always present; for any non-compound
type, it is \code{empty\_path}.  Additionally, \code{empty\_path} is used
for a compound type when the annotation is on its main part.  For example,
consider a one-dimensional array type.  An annotation on the array type
itself would use \code{empty\_path}.  An annotation on the array element
type would use \code{array\_type\_path}, within which would be an
\code{empty\_path}.
