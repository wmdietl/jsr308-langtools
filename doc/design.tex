...
incorporation in the Oracle JDK (Section~\ref{logistics})
and
...
type, the children of the parameterized (``raw'') type are the type arguments.  For an array
...
\myurl{http://download.oracle.com/javase/6/docs/api/javax/lang/model/element/package-summary.html}.
...
% In appendix D, for each proposal, state its impact on
%  * grammar
%  * class file
%  * reflection
...
reference implementation has been incorporated into Oracle's OpenJDK Java
implementation.  Oracle will integrate the specification into the JLS (without
any conceptual changes or new information), will write additional tests,
etc.  We do not have control over the license terms that Oracle places on its
changes.
