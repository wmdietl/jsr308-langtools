...
in an annotation (\<@\emph{Foo}>), \<throws> clause, or \<catch> clause.
...
The \nt{CatchType} rule implies that it is possible to write annotations on
each disjunct of a multi-catch.  However, there is no syntactic sugar that
permits a programmer to write an annotation that applies to each disjunct; the
programmer would have to duplicate the annotation for each disjunct.
