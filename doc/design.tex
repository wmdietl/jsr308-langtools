...
\item
  ...
  name, as in \code{Outer. @NonNull StaticNested} or \code{myVar. @NonNull
    Inner}.  If the nested type is a static type, then both the nested and
...
\item
  ...
  name or wildcard, as in \code{class MyClass<@Immutable T> \ttlcb\ ... \ttrcb}.
\item
  An annotation on a disjunctive type in a multi-\<catch> clause appears
  before the opening parenthesis, and is treated as syntactic sugar for
  writing the type annotation on each disjunct.
...
\item
Annotations may appear before the parenthesis of a multi-\<catch>:

\begin{tabbing}
\nt{CatchClause}: \\
\qquad \= \kill
\qquad        \term{catch} \new{\opt{\nt{Annotations}}} \term{(} \multi{\nt{VariableModifier}} \nt{CatchType} \nt{Identifier} \term{)} \nt{Block}
\end{tabbing}
...
class type parameter                            & \code{0x22}, \code{0x23}\rlunused & \S\ref{class-file:ext:ri:mtp} \\
method type parameter                           & \code{0x20}, \code{0x21}\rlunused & \S\ref{class-file:ext:ri:mtp} \\
\hline
class \code{extends}/\code{implements}          & \code{0x14}, \code{0x15} & \S\ref{class-file:ext:ri:extends} \\
class type parameter bound                      & \code{0x10}, \code{0x11} & \S\ref{class-file:ext:ri:tpbound} \\
method type parameter bound                     & \code{0x12}, \code{0x13} & \S\ref{class-file:ext:ri:tpbound} \\
field                                           & \code{0x0E}, \code{0x0F} & \S\ref{class-file:ext:ri:return} \\
method return type                              & \code{0x0A}, \code{0x0B} & \S\ref{class-file:ext:ri:return} \\
method receiver                                 & \code{0x06}, \code{0x07} & \S\ref{class-file:ext:ri:receiver} \\
method parameter                                & \code{0x0C}, \code{0x0D} & \S\ref{class-file:ext:ri:parameters} \\
exception type in \code{throws}                 & \code{0x16}, \code{0x17}\rlunused & \S\ref{class-file:ext:ri:throws} \\
wildcard bound                                  & \code{0x1C}, \code{0x1D} & \S\ref{class-file:ext:ri:wildcard} \\
\hline
local variable                                  & \code{0x08}, \code{0x09} & \S\ref{class-file:ext:ri:localvar} \\
resource variable                               & \code{TO DO}, \code{TO DO} & \S\ref{class-file:ext:ri:localvar} \\
exception parameter                             & \code{TO DO}, \code{TO DO} & \S\ref{class-file:ext:ri:localvar} \\
typecast                                        & \code{0x00}, \code{0x01} & \S\ref{class-file:ext:ri:typecast} \\
type test (\code{instanceof})                   & \code{0x02}, \code{0x03} & \S\ref{class-file:ext:ri:typecast} \\
object creation (\code{new})                    & \code{0x04}, \code{0x05} & \S\ref{class-file:ext:ri:typecast} \\
type argument in constructor call               & \code{0x18}, \code{0x19} & \S\ref{class-file:ext:ri:con-typearg} \\
type argument in method call                    & \code{0x1A}, \code{0x1B} & \S\ref{class-file:ext:ri:con-typearg} \\
\hline
\end{tabular}
\end{center}
%BEGIN LATEX
...
the \code{ldc}(\code{\_w}) bytecode emitted for class literals, or the
...
the class file; that is OK, becauuse it is irrelevant to the program.
...
location among the type arguments and array levels.
...
type, the children of the parameterized type are the type arguments.  For an array
...
language~\cite[ch.~18]{GoslingJSBB2011}, based on the conceptually
...
% because (declaration) annotations are already permitted there.
 \\
\nt{MethodOrFieldDecl}: \\
\qquad       \nt{\new{Unann}Type} \nt{Identifier} \nt{MethodOrFieldRest} \\
 \\
...
Sections~\ref{type-annotation-use-cases}.
...
%   List<String>.class
% because Java 7 didn't permit that.  It should perhaps be fixed.
...
However, special syntax is provided for the disjunctive types that arise
...
\subsection{Not all type names are annotatable\label{type-names}}
...
An annotation use cannot itself be annotated.  (An annotation declaration
...
\noindent
A static member access may itself be a type use, in which cane the used
...
\item
  ...
  meta-annotated with \<@Target({ ElementType.TYPE\_USE, ElementType.METHOD })>
   or \<@A> has no \<@Target> meta-annotation; in ethier case, the \<@A>
...
% LocalWords:  FormalParameters VoidMethodDeclaratorRest BracketsOpt
