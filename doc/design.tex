...
The JSR 308 webpage is \myurl{http://groups.csail.mit.edu/pag/jsr308/}.
It contains the
\ahref{http://groups.csail.mit.edu/pag/jsr308/specification/java-annotation-design.html}{latest version}
% latest version
of this document, along
with other information such as a FAQ, the reference implementation, and
sample annotation processors.

%HEVEA This document is available in PDF format at \myurl{http://groups.csail.mit.edu/pag/jsr308/specification/java-annotation-design.pdf}.
...
see \myurl{http://pag.csail.mit.edu/jsr308/jsr308-faq.html}.
...
Also permit annotations on varargs (\code{...}):
...
(\myurl{http://groups.csail.mit.edu/pag/jsr308/current/checkers-manual.html}) is
...
size and
contents of \code{reference\_info} (see Section~\ref{class-file:ext:reference_info}).
...
method receiver generic/array                   & \code{0x07}\rlunused & Future extension to JSR 308 \\ % +\S\ref{class-file:ext:ri:typearg}
...
\subsection{The \code{reference\_info} field\label{class-file:ext:reference_info}}

\code{reference\_info} is a structure that contains enough information to
...
of the \code{reference\_info} is determined by the value of
\code{target\_type}.

% , and in some cases the size of \code{reference\_info} is zero ...



\subsubsection{Typecasts, type tests, object creation and class literal\label{class-file:ext:ri:typecast}}

When the annotation's target is a typecast, an \code{instanceof} expression, a \code{new} expression, or a class literal expression
\code{reference\_info} has
...
the typecast, the \code{instanceof} bytecode emitted for the type tests, or of the \code{new} bytecode
...
underlying Java type system (which sees a cast from \code{String}
...
\code{reference\_info} has the following structure:
...
When the annotation's target is a method parameter, \code{reference\_info}
...
The \code{location\_length} and \code{location} fields are as in
...
constructor receiver), \code{reference\_info}
is empty.
...
method, \code{reference\_info} has the following structure:
...
a method call, \code{reference\_info} has the following structure:
...
\code{implements} clause, \code{reference\_info} has the following
structure:
...
\code{type\_index} specifies the index of the type in the clause:
\code{-1} (\code{255}) is used if the annotation is on the superclass
...
array type,
\code{reference\_info} contains what it normally would for the raw type
...
The \code{location\_length} field specifies the number of elements in
the variable-length \code{location} field. \code{location} encodes
1which type argument or array element the annotation targets.
Specifically, the $i$th item in \code{location} denotes the index of
the type argument or array dimension at the $i$th level of the
hierarchy.  Figure~\ref{tbl:locations} shows the values of the
\code{location\_length} and \code{location} fields for the annotations
in a sample field declaration.



% Mahmood says:  By the way, the compiler fails when parsing such types.
TODO:  The specification does not indicate how to differentiate \code{@A} and
...
% , and and deletions \sout{crossed out}.
...
\myurl{http://groups.csail.mit.edu/pag/jsr308/}.  The four checkers, respectively,
...
(\myurl{http://groups.csail.mit.edu/pag/jsr308/current/checkers-manual.html})
...
parameters.
% "does not currently" was written 9/26/2008, still true 2/26/2009
...
% The IGJ type system~\cite{ZibinPAKE2007} has been implemented using both a
...
\code{ElementType.FIELD} in order to be applied to a class, a method return
...
A related case is that when annotation has no \code{@Target}
...
\noindent
the \code{@Deprecated} annotation applies to both the \code{int} data type
and to the \code{foo} method.  Any annotation processor that processes the
\code{@Deprecated} annotation will not even look in the type annotation
...
%  new syntactic location
%  @ReceiverNonNull
%  @ @NonNull Receiver   <-- this is legal!
%  @Receiver(@NonNull)
...
require the \code{javac} Java compiler to accept annotations in the proposed
...
types'' (\myurl{http://groups.csail.mit.edu/pag/jsr308/java-type-qualifiers.pdf}),
...
(\myurl{http://groups.csail.mit.edu/pag/jsr308/annotation-file-utilities/annotation-file-format.pdf}), describes a
...
\paragraph{The last annotations JSR.}
It is not a goal that JSR 308 is the last annotation-related JSR\@.  It is
acceptable to leave some issues to future language designers, just as
...
\code{@AContainer} annotations are present.
...
\item
Add new methods that return multiple annotations.

\begin{enumerate}
\item
Each method of the form
...
\noindent
would have its specification slightly modified.  When
duplicate annotations exist, method
\code{getAnnotation} could either give the first one (convenient for
backward compatibility) or throw an exception (convenient to prevent
erroneous processing).

The relevant methods are in \code{java.lang.reflect.AnnotatedElement} and
its 6 implementations, and in \code{javax.lang.model.element.Element}.
...
\item
Each \code{getAnnotation} method 
would be augmented by a new method:
...
present on the receiver element.  As with \code{getAnnotations}, the return
value is never null but may have zero length.
...
The JSR 175 Design FAQ~\cite{JSR175-PFD2} states,

\begin{quote}
\textbf{Why is it illegal [to] annotate a single element repeatedly with the same annotation type?}
...
the class file.  See
\myurl{http://doc.ece.uci.edu/mediawiki/index.php/JSR-308_Statements} for a
...
\code{jsr308-statements@lists.csail.mit.edu} mailing list (via
\myurl{https://lists.csail.mit.edu/mailman/listinfo/jsr308-statements/}),
...
\begin{quote}
  \textbf{Why can't you annotate arbitrary program elements such as blocks
...
framework that defines annotation \code{@A} will not want to load an
untrusted subclass \code{@B} of \code{@A} into the framework.

% Suppose annotation \code{@A} is tied to a framework.  If
% modifier on an annotation type declaration prevents creation of subtypes
% of the given annotation, just as for ordinary classes.  A framework that
% defines annotation \code{@A} will not want to load an untrusted subclass
% \code{@B} of \code{@A} into the framework.  If
% someone creates \code{@B}, a subclass of annotation
% \code{@A}, then by the Liskov Substitution Principle, \code{@B} must
% function as \code{@A}.  But the framework will not want to load the
% subclass \code{@B} into its VM, as \code{@B} is alien and untrusted code
...
\code{Sub1} and \code{Sub2} are subtypes of \code{Super} which has an
\code{int} value field and a location is annotated with both
\code{@Sub1(1)} and \code{@Sub2(2)}.
The old \code{getAnnotation(Class)} method should throw an error in this case.
...
% That processor may need to handle the possibility of multiple annotations
% even for an annotation that is not permitted to

The JSR 175 Design FAQ~\cite{JSR175-PFD2} states, without further
elaboration,

\begin{quote}
\textbf{Why don't you support annotation subtyping (where one annotation type
extends another)?}
...
\begin{quote}
  \textbf{Why is it illegal for an annotation type to contain an element of the same type?}
...
\subsubsection{Reducing class file size via use of the constant pool}
...
\subsubsection{Optional/default fields in annotations}
...
(\myurl{https://lists.csail.mit.edu/mailman/listinfo/jsr308/}) provided
...
% LocalWords:  getReturnAnnotation getReturnAnnotations getAnnotation enums
...
% LocalWords:  Nonnegative typedefs unabbrev reusability typedef
